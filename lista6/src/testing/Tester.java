...
public class Tester {
	...
	public static <T> RunResult runOnce(SortingAlgorithm<MarkedValue<T>> algorithm, Generator<MarkedValue<T>> generator, int size) {
		...
		timer.stop();
		return new RunResult(timer.durationMillis(), algorithm.comparisons(), algorithm.swaps(), algorithm.adds(),
	...
	public static <T> Result runNTimes(SortingAlgorithm<MarkedValue<T>> algorithm, Generator<MarkedValue<T>> generator, 
		...
		double averageAdds = 0.0;
		double averageAddsSquared = 0.0;
		...
		for(int n = 1; n <= repetitions; ++n) {
			...
			averageAdds = updatedAverage(averageAdds, result.adds(), n);
			averageAddsSquared = updatedAverage(averageAddsSquared, (double)result.adds() * (double)result.adds(), n);
		...
		return new Result(averageTime, calculateStdDev(averageTime, averageTimeSquared), 
				...
				averageAdds, calculateStdDev(averageAdds, averageAddsSquared),
				sorted, stable);
	}
